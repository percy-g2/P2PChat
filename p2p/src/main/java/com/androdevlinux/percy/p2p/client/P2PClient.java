...
import com.androdevlinux.percy.p2p.common.WiFiDirectBroadcastReceiver;
 ...
 * {@link P2PService} can register a service which could be discover by the multiple client
 ...
 * <pre>
 * {@code wiFiP2PClient = P2PClient.getInstance(getApplicationContext());
 ...
 * {@link #connectToService(P2PDeviceService, ServiceConnectedListener)} passing as argument the
 * appropiate {@link P2PDeviceService} obtained in the <code>discoverServices()</code> call.
 */
public class P2PClient implements PeerConnectedListener, ServiceDisconnectedListener {

    private static final String TAG = P2PClient.class.getSimpleName();

    private static P2PClient instance;

    private List<P2PDeviceService> serviceDevices = new ArrayList<>();
    ...
    private P2PDevice serviceDevice;
    private Map<String, P2PDevice> clientsConnected;
     ...
     */
    public static P2PClient getInstance(Context context) {
        if (instance == null && context != null) {
            instance = new P2PClient(context);
        }
        return instance;
    }
    ...
    /**
     * Start the connection with the <code>P2PDeviceService</code> passed by argument. When the
     * connection is stablished with the device service the {@link ServiceConnectedListener#onServiceConnected(P2PDevice)}
     * method is called.
     * <p>
     ...
     *                                 the desired service.
     */
    public void connectToService(final P2PDeviceService serviceDevice, ServiceConnectedListener serviceConnectedListener) {
    ...
    public void sendMessageToAllClients(MessageWrapper message) {
        ...
        for (P2PDevice device : clientsConnected.values()) {
    ...
    public void sendMessage(final P2PDevice device, MessageWrapper message) {
     ...
     */
    public Collection<P2PDevice> getClientsConnected() {
    ...
    private DnsSdTxtRecordListener getTxtRecordListener(final ServiceDiscoveredListener serviceDiscoveredListener) {
        return new DnsSdTxtRecordListener() {
            ...
            public void onDnsSdTxtRecordAvailable(String fullDomainName, Map<String, String> txtRecordMap, WifiP2pDevice device) {
                ...
                if (txtRecordMap.containsKey(P2PService.SERVICE_NAME_PROPERTY) && txtRecordMap.get(P2PService.SERVICE_NAME_PROPERTY).equalsIgnoreCase(P2PService.SERVICE_NAME_VALUE)) {
                    Integer servicePort = Integer.valueOf(txtRecordMap.get(P2PService.SERVICE_PORT_PROPERTY));
                    P2PDeviceService serviceDevice = new P2PDeviceService(device);
    ...
    private void onMessageReceived(MessageWrapper messageWrapper) {
        if (MessageWrapper.MessageType.CONNECTION_MESSAGE.equals(messageWrapper.getMessageType())) {
            ...
            P2PDevice device = registrationMessageContent.getWroupDevice();
            ...
            P2PDevice device = disconnectionMessageContent.getWroupDevice();
            ...
            List<P2PDevice> devicesConnected = registeredDevicesMessageContent.getDevicesRegistered();

            for (P2PDevice device : devicesConnected) {
                clientsConnected.put(device.getDeviceMac(), device);
